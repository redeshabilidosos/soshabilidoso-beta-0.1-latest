...
// Mock data para los clips (fallback)
const mockReels = [
  {
    id: '1',
...
export default function ClipsPage() {
  ...
  // Función para cargar clips desde la API
  const fetchReels = useCallback(async (pageNum: number = 1) => {
    try {
      ...
      console.log(`📡 Cargando clips - Página ${pageNum}...`);
      ...
      console.log('✅ Clips cargados:', data);
      ...
      if (data.results && data.results.length > 0) {
        ...
        // Si no hay clips en la primera página, usar mock data
        console.log('ℹ️ No hay clips, usando mock data');
        setReels(mockReels);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Solo cargar clips si hay usuario autenticado
    if (user) {
      fetchReels(1);
    }
  }, [user, authLoading, router, fetchReels]);

  // Detectar ID en URL y posicionar el clip correcto
  useEffect(() => {
    if (reels.length > 0) {
      ...
      if (reelId) {
        ...
        if (reelIndex !== -1) {
          ...
          window.history.replaceState({}, '', '/clips');
        }
      }
    }
  }, [reels]);

  // Cargar más clips cuando se acerque al final
  ...
  const handleComment = (reelId: string) => {
    console.log('Comment on clip:', reelId);
  ...
  const handleShare = async (reel: any) => {
    ...
    try {
      ...
      // Compartir con URL única del clip
      const shareUrl = `${window.location.origin}/clips?id=${reel.id}`;
  ...
  const handleFollow = async (userId: string) => {
    try {
      ...
      // Encontrar el username del usuario en los clips
  ...
  // Mostrar loading de clips solo si hay usuario pero aún cargando clips
  ...
  const handleIndexChange = (newIndex: number) => {
    ...
    // Cargar más cuando esté a 2 clips del final
      ...
      <main className="lg:ml-64">
        ...
        {isFetchingMore && (
          <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-50 bg-black/80 px-4 py-2 rounded-full">
            <p className="text-white text-sm">Cargando más clips...</p>
          </div>
        )}
      </main>
